...
1. Reuse the existing **Bingo Halls and Texting** project, `gdvzebqxecmanobdbhha`, at `https://gdvzebqxecmanobdbhha.supabase.co`. Do not create another project or reset its data.
2. The original schema migration is already applied. Inspect actual tables, policies, and functions before considering any migration; the SQL Editor application may not appear in the migration ledger. Do not rerun setup or scheduling SQL as part of folder repair.
